...
function formatTenantSyncResult(result: AppStoreConnectTenantSyncResult): string {
  const sales =
    result.salesReport == null
      ? ''
      : ` Sales Report ${result.salesReport.reportDate}: ${result.salesReport.status}.`
...
export function WorkspaceSettingsView({
  ...
  const [draft, setDraft] = React.useState<AppStoreConnectCredentialDraft>(() =>
    credentialDraftFromConnection(connection),
  )
  ...
  const saveCredential = () => {
    runTask('save', async () => {
      const result = await saveAppStoreConnectCredential({
        data: { ...draft, tenantId: tenant.id },
      })
  ...
  const listAccessibleApps = () => {
    runTask('list-apps', async () => {
      const result = await listAppStoreConnectApps({
        data: connection?.hasPrivateKey
          ? { tenantId: tenant.id }
  ...
  return (
    <section className="max-w-[1080px] animate-[subkit-fade-in_200ms_ease] px-[32px] py-[28px] max-md:px-[18px]">
      ...
      <div className="mt-[20px] grid grid-cols-[1.15fr_0.85fr] gap-[16px] max-lg:grid-cols-1">
        ...
        >
          <AppStoreConnectCredentialForm
            draft={draft}
            hasStoredKey={connection?.hasPrivateKey ?? false}
          ...
          <div className="flex flex-wrap gap-[8px]">
            <ActionButton
              ...
              label={
                busy === 'save'
                  ? 'Saving & syncing…'
                  : connection == null
                    ? 'Save, validate & sync key'
                    : 'Save, rotate & sync key'
              }
            ...
            <ActionButton
              disabled={busy != null}
              label="List accessible apps"
              onPress={listAccessibleApps}
            />
            <ActionButton
              disabled={busy != null || connection == null || !canManageTenant}
              label="Run preflight"
              onPress={validateCredential}
            />
            <ActionButton
              disabled={busy != null || connection == null || !canManageTenant}
              label="Delete workspace key"
              onPress={deleteCredential}
              tone="danger"
            />
          </div>
          {feedback != null ? (
            <Notice className="m-0" tone={feedback.tone}>
              {feedback.message}
            </Notice>
          ) : null}
        ...
        <SettingsCard
          description="What this credential can do right now."
          title="Connection health"
        >
          {connection == null ? (
            <EmptySettingsText>
              No App Store Connect credential is configured yet.
            </EmptySettingsText>
          ) : (
            <>
      ...
      <SettingsCard
        description="Customer-visible history for credential actions. Secret values are redacted."
        title="Audit log"
      >
