...
import type { AppStoreConnectCredentialDraft } from '~/integrations/app-store-connect/types'
...
export function AppStoreConnectCredentialForm({
  ...
  return (
    <div className="flex flex-col gap-[12px]">
      <div className="grid grid-cols-2 gap-[12px] max-sm:grid-cols-1">
        ...
        <PUIField label="Issuer ID">
          {(field) => (
            <PUIInput
              className="font-mono"
              onChange={(event) => onChange('issuerId', event.target.value)}
        ...
        <PUIField hint="Required for Sales & Trends reports." label="Vendor Number">
          {(field) => (
            <PUIInput
              className="font-mono"
              onChange={(event) => onChange('vendorNumber', event.target.value)}
              placeholder="12345678"
              value={draft.vendorNumber}
              {...inputFieldProps(field)}
            />
          )}
        </PUIField>
        <div className="rounded-[11px] border border-[var(--subkit-border)] bg-[var(--subkit-panel-2)] p-[12px] text-[12.5px] text-[var(--subkit-dim)]">
          ...
          <p className="mt-[4px] mb-0 leading-[1.45]">
            Start with read access. Sales reports, TestFlight, provisioning, and release metadata
      ...
      <div>
        <label
          className="mb-[7px] block text-[12.5px] font-semibold text-[var(--subkit-text)]"
          htmlFor="app-store-connect-private-key"
        >
          Private .p8 key file
        </label>
        <div className="rounded-[12px] border border-dashed border-[var(--subkit-border-2)] bg-[var(--subkit-panel-2)] p-[14px]">
          ...
          {privateKeyUploadError != null ? (
            <div className="mt-[6px] text-[12px] font-semibold text-[var(--subkit-red)]">
