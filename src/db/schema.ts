...
export const users = sqliteTable(
  'users',
  {
    ...
    globalRole: text('global_role', { enum: ['user', 'super_admin'] })
      .notNull()
      .default('user'),
...
export const apps = sqliteTable(
  'apps',
  {
    ...
    status: text('status', { enum: ['setup', 'live', 'beta', 'inactive'] })
      .notNull()
      .default('setup'),
...
export const appPlatforms = sqliteTable(
  'app_platforms',
  {
    ...
    environment: text('environment', { enum: ['production', 'sandbox', 'internal', 'unknown'] })
      .notNull()
      .default('unknown'),
    status: text('status', { enum: ['connected', 'needs_setup', 'disabled'] })
      .notNull()
      .default('needs_setup'),
    createdAt: integer('created_at', { mode: 'timestamp_ms' })
      .notNull()
  ...
  (table) => [
    uniqueIndex('app_platforms_app_store_environment_unique').on(
      table.appId,
      table.store,
      table.environment,
    ),
...
export const storeIntegrations = sqliteTable(
  'store_integrations',
  {
    ...
    appId: text('app_id')
      .notNull()
      .references(() => apps.id, { onDelete: 'cascade' }),
    appPlatformId: text('app_platform_id').references(() => appPlatforms.id, {
      onDelete: 'set null',
    }),
    store: text('store', { enum: ['apple', 'google'] }).notNull(),
    displayName: text('display_name').notNull(),
    status: text('status', { enum: ['connected', 'needs_auth', 'disabled', 'error'] })
      .notNull()
      .default('needs_auth'),
...
export const appStoreConnectCredentials = sqliteTable(
  'app_store_connect_credentials',
  {
    ...
    status: text('status', { enum: ['connected', 'needs_attention', 'invalid', 'deleted'] })
      .notNull()
...
export const appStoreConnectCapabilities = sqliteTable(
  ...
  (table) => [
    uniqueIndex('app_store_connect_capabilities_credential_key_unique').on(
      table.credentialId,
      table.key,
    ),
  ],
...
export const appStoreConnectSalesReports = sqliteTable(
  ...
  (table) => [
    index('app_store_connect_sales_reports_credential_created_idx').on(
      table.credentialId,
      table.createdAt,
    ),
  ],
...
export const appStoreConnectAuditEvents = sqliteTable(
  'app_store_connect_audit_events',
  {
    ...
    tenantId: text('tenant_id')
      ...
      .references(() => tenants.id, { onDelete: 'cascade' }),
    appId: text('app_id').references(() => apps.id, { onDelete: 'cascade' }),
    credentialId: text('credential_id').references(() => appStoreConnectCredentials.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => [
    index('app_store_connect_audit_events_credential_created_idx').on(
      table.credentialId,
      table.createdAt,
    ),
...
export const products = sqliteTable(
  'products',
  {
    ...
    productType: text('product_type', {
      enum: ['subscription', 'non_consumable', 'consumable', 'voucher', 'manual'],
    })
      .notNull()
...
export const productEntitlements = sqliteTable(
  'product_entitlements',
  {
    ...
    grantMode: text('grant_mode', { enum: ['while_active', 'lifetime', 'fixed_duration'] })
      .notNull()
      .default('while_active'),
  ...
  (table) => [
    uniqueIndex('product_entitlements_product_entitlement_unique').on(
      table.productId,
      table.entitlementId,
    ),
  ],
...
export const prices = sqliteTable(
  ...
  (table) => [
    uniqueIndex('prices_plan_currency_country_unique').on(
      table.productPlanId,
      table.currencyCode,
      table.countryCode,
    ),
...
export const productOffers = sqliteTable(
  'product_offers',
  {
    ...
    offerType: text('offer_type', {
      enum: ['free_trial', 'intro_price', 'promo', 'winback', 'custom_code'],
    }).notNull(),
    eligibility: text('eligibility', {
      enum: ['new_customers', 'lapsed', 'existing', 'all', 'store_managed'],
    })
      .notNull()
...
export const storeProductBindings = sqliteTable(
  'store_product_bindings',
  {
    ...
    productPlanId: text('product_plan_id')
      .notNull()
      .references(() => productPlans.id, { onDelete: 'cascade' }),
    storeIntegrationId: text('store_integration_id').references(() => storeIntegrations.id, {
      onDelete: 'set null',
    }),
    store: text('store', { enum: ['apple', 'google'] }).notNull(),
    ...
    environment: text('environment', { enum: ['production', 'sandbox', 'unknown'] })
      .notNull()
      .default('production'),
    ...
    syncDirection: text('sync_direction', {
      enum: ['subkit_to_store', 'store_to_subkit', 'manual'],
    })
      .notNull()
      .default('subkit_to_store'),
  ...
  (table) => [
    uniqueIndex('store_product_bindings_store_product_unique').on(
      table.appId,
      table.store,
      table.externalProductId,
      table.externalBasePlanId,
      table.environment,
    ),
  ],
)
...
export const storeCatalogDriftItems = sqliteTable('store_catalog_drift_items', {
  ...
  appId: text('app_id')
    .notNull()
    .references(() => apps.id, { onDelete: 'cascade' }),
  storeProductBindingId: text('store_product_binding_id').references(
    () => storeProductBindings.id,
    { onDelete: 'set null' },
  ),
  snapshotId: text('snapshot_id').references(() => storeCatalogSnapshots.id, {
    onDelete: 'set null',
  }),
  ...
  status: text('status', { enum: ['open', 'acknowledged', 'resolved', 'ignored'] })
    .notNull()
    .default('open'),
...
export const syncRuns = sqliteTable('sync_runs', {
  ...
  store: text('store', { enum: ['apple', 'google'] }).notNull(),
  mode: text('mode', {
    enum: ['import', 'compare', 'plan', 'apply', 'verify', 'sales_import'],
  }).notNull(),
  status: text('status', {
    enum: ['running', 'succeeded', 'failed', 'partial', 'cancelled'],
  }).notNull(),
...
export const storeMutationPlans = sqliteTable('store_mutation_plans', {
  ...
  risk: text('risk', { enum: ['none', 'low', 'medium', 'high', 'irreversible'] })
    .notNull()
    .default('none'),
})
...
export const storeMutationPlanItems = sqliteTable('store_mutation_plan_items', {
  ...
  status: text('status', { enum: ['planned', 'applied', 'failed', 'skipped', 'blocked'] })
    .notNull()
    .default('planned'),
  errorDetail: text('error_detail'),
...
export const offerings = sqliteTable('offerings', {
  ...
  tagTone: text('tag_tone', { enum: ['success', 'warning', 'muted', 'destructive'] })
    .notNull()
    .default('muted'),
})
...
export const appUserStoreIdentities = sqliteTable(
  ...
  (table) => [
    uniqueIndex('app_user_store_identities_user_store_unique').on(
      table.appId,
      table.appUserId,
      table.store,
    ),
    uniqueIndex('app_user_store_identities_apple_token_unique').on(
      table.appId,
      table.appAccountToken,
    ),
    uniqueIndex('app_user_store_identities_google_account_unique').on(
      table.appId,
      table.obfuscatedAccountId,
    ),
  ],
)
...
export const entitlementGrants = sqliteTable(
  'entitlement_grants',
  {
    ...
    productPlanId: text('product_plan_id').references(() => productPlans.id, {
      onDelete: 'set null',
    }),
    storeProductBindingId: text('store_product_binding_id').references(
      () => storeProductBindings.id,
    ...
    source: text('source', {
      enum: ['apple', 'google', 'voucher', 'promo', 'manual', 'lifetime', 'migration'],
    }).notNull(),
    status: text('status', {
      enum: ['active', 'trialing', 'billing_retry', 'expired', 'revoked'],
    }).notNull(),
...
export const storePurchaseOwnerships = sqliteTable(
  'store_purchase_ownerships',
  {
    ...
    entitlementGrantId: text('entitlement_grant_id').references(() => entitlementGrants.id, {
      onDelete: 'set null',
    }),
    store: text('store', { enum: ['apple', 'google'] }).notNull(),
    ...
    environment: text('environment', { enum: ['sandbox', 'production', 'unknown'] })
      .notNull()
      .default('unknown'),
    ownershipType: text('ownership_type', { enum: ['purchased', 'family_shared', 'unknown'] })
      .notNull()
      .default('unknown'),
    purchaseTokenHash: text('purchase_token_hash'),
    receiptHash: text('receipt_hash'),
    status: text('status', {
      enum: ['active', 'trialing', 'billing_retry', 'expired', 'revoked', 'pending'],
    })
      .notNull()
      .default('pending'),
  ...
  (table) => [
    uniqueIndex('store_purchase_ownerships_store_original_unique').on(
      table.appId,
      table.store,
      table.originalTransactionId,
    ),
    uniqueIndex('store_purchase_ownerships_store_transaction_unique').on(
      table.appId,
      table.store,
      table.transactionId,
    ),
...
export const runtimeReconcileEvents = sqliteTable('runtime_reconcile_events', {
  ...
  storePurchaseOwnershipId: text('store_purchase_ownership_id').references(
    () => storePurchaseOwnerships.id,
    { onDelete: 'set null' },
  ),
  store: text('store', { enum: ['apple', 'google'] }).notNull(),
  action: text('action').notNull(),
