  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const parsed = runtimeCustomerInfoRequestSchema.safeParse(await request.json().catch(() => null))
          if (!parsed.success) {
            return jsonApiError({ code: 'invalid_request', details: parsed.error.flatten(), message: 'Invalid runtime customer info request', status: 400 })
          }
