...
export function appStatus(status: 'setup' | 'live' | 'beta' | 'inactive'): {
  label: string
  tone: StatusTone
} {
...
export function storeSyncObjectLabel(
  ...
  if (binding != null) {
    ...
    return basePlan === ''
      ? `${binding.store}:${binding.externalProductId}`
      : `${binding.store}:${binding.externalProductId}:${basePlan}`
  }
...
export function sortGrantsByRelevance<
  T extends { createdAt: Date; status: EntitlementGrantStatus },
>(items: readonly T[]): T[] {
...
export function revenueBarsForEvents(
  events: readonly (typeof purchaseEvents.$inferSelect)[],
): RevenueBar[] {
  ...
  return values.map(([month, cents]) => ({
    height: `${Math.max(1, Math.round((cents * 100) / maxRevenue))}%`,
    month,
    value: formatCurrency(cents),
  }))
}

export function appPlatformLabels(
...
export function appStoreSummary(
  appleAppId: string | null,
  iosBundleId: string | null,
  bundleId: string,
  androidPackageName: string | null,
