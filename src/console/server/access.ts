...
export function toWorkspaceTenant(tenant: typeof tenants.$inferSelect, role: WorkspaceTenant['role']): WorkspaceTenant {
  return {
    color: tenant.color,
    id: tenant.id,
...
export async function tenantRolesById(user: AuthUser, tenantIds: readonly string[]): Promise<Map<string, WorkspaceTenant['role']>> {
...
export function roleForTenant(roleByTenantId: ReadonlyMap<string, WorkspaceTenant['role']>, tenantId: string): WorkspaceTenant['role'] {
  return roleByTenantId.get(tenantId) ?? 'developer'
}

export function canCreateTenants(user: AuthUser, roleByTenantId: ReadonlyMap<string, WorkspaceTenant['role']>): boolean {
  return isSuperAdmin(user) || [...roleByTenantId.values()].some((role) => role === 'admin')
}

export function toTenantMemberSummary(row: { createdAt: Date; role: TenantRole; tenantId: string; user: typeof users.$inferSelect }): TenantMemberSummary {
  return {
...
export async function assertTenantKeepsAdmin(tenantId: string, excludedUserId: string): Promise<void> {
...
export async function requireAccessibleApp(user: AuthUser, appId: string): Promise<typeof apps.$inferSelect> {
