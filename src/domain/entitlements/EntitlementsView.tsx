...
export function EntitlementsView({ entitlements }: { entitlements: Entitlement[] }) {
  return (
    <section className="max-w-[980px] animate-[subkit-fade-in_200ms_ease] px-[32px] py-[28px] max-md:px-[18px]">
      <ViewTitle
        description="What a user unlocks. Check entitlements in your app, never product IDs."
        title="Entitlements"
      />
      <div className="mt-[20px] flex flex-col gap-[14px]">
        {entitlements.length === 0 ? (
          <EmptySettingsText>
            No entitlements yet. Create a product first; SubKit will create or reuse the entitlement
            key that your app backend should check.
          </EmptySettingsText>
        ) : null}
          ...
          >
            <div className="flex items-center gap-[12px]">
              <div className="flex size-[40px] items-center justify-center rounded-[10px] border border-[var(--subkit-accent-line)] bg-[var(--subkit-accent-soft)]">
                <Circle
                  aria-hidden
                  className="size-[18px] fill-[var(--subkit-accent)] text-[var(--subkit-accent)]"
                />
              </div>
              <div className="flex-1">
                <div className="font-mono text-[15px] font-semibold">{entitlement.id}</div>
                <div className="mt-[2px] text-[13px] text-[var(--subkit-dim)]">
                  {entitlement.description}
                </div>
              </div>
              <div className="text-[12px] text-[var(--subkit-faint)]">
                {entitlement.productCount}
              </div>
            </div>
