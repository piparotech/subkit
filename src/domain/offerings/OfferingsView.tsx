...
export function OfferingsView({ offerings }: { offerings: Offering[] }) {
  return (
    <section className="max-w-[1080px] animate-[subkit-fade-in_200ms_ease] px-[32px] py-[28px] max-md:px-[18px]">
      <ViewTitle
        description="The set of packages shown on a paywall. Change pricing without an app release."
        title="Offerings"
      />
      <div className="mt-[20px] flex flex-col gap-[16px]">
        {offerings.length === 0 ? (
          <EmptySettingsText>
            No offerings yet. Offerings group products into the packages your paywall can request
            without shipping an app release.
          </EmptySettingsText>
        ) : null}
        {offerings.map((offering) => (
          <PUICard
            className="rounded-[14px] border-[var(--subkit-border)] bg-[var(--subkit-panel)] px-[20px] py-[18px]"
            key={offering.id}
          >
            <div className="mb-[16px] flex items-center gap-[10px] max-md:flex-wrap">
              <div className="text-[15px] font-semibold">{offering.name}</div>
              <span className="font-mono text-[11.5px] text-[var(--subkit-faint)]">
                {offering.id}
              </span>
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-[12px]">
              {offering.packages.map((pkg) => (
                <div
                  className="relative rounded-[11px] border border-[var(--subkit-border)] bg-[var(--subkit-panel)] p-[14px]"
                  key={pkg.productId}
                >
                  {pkg.hasBadge ? (
                    <span className="absolute top-[-9px] left-[12px] rounded-[5px] bg-[var(--subkit-accent)] px-[7px] py-[2px] text-[10px] font-bold text-white">
                      {pkg.badge}
                    </span>
                  ) : null}
