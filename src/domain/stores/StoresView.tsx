...
export function StoresView({ storeSync }: { storeSync: StoreSyncAppSummary }) {
  ...
  const blockingDriftCount = storeSync.driftItems.filter(
  ...
  return (
    <section className="animate-[subkit-fade-in_200ms_ease] px-[32px] py-[28px] max-md:px-[18px]">
      ...
      <div className="mt-[14px] rounded-[12px] border border-[var(--subkit-accent-line)] bg-[var(--subkit-accent-soft)] px-[14px] py-[11px] text-[12.5px] leading-[1.45] text-[var(--subkit-accent-d)]">
        Store rows are projections. SubKit remains canonical for product, plan, entitlement, and
        offering intent. Store writes require a separate preview and explicit confirmation flow.
      </div>
      ...
      <div className="mt-[18px] grid grid-cols-4 gap-[12px] max-lg:grid-cols-2 max-sm:grid-cols-1">
        ...
        <StoreMetric
          label="Open drift"
          statusLabel={openDriftCount > 0 ? 'Review' : 'Clear'}
          tone={openDriftCount > 0 ? 'warning' : 'success'}
          value={String(openDriftCount)}
        />
        <StoreMetric
          label="Blocking drift"
          statusLabel={blockingDriftCount > 0 ? 'Blocked' : 'Clear'}
          tone={blockingDriftCount > 0 ? 'destructive' : 'success'}
          value={String(blockingDriftCount)}
        />
      </div>
      ...
      <section className="mt-[20px] grid grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)] gap-[16px] max-xl:grid-cols-1">
        <StorePanel title="Product bindings">
          {storeSync.bindings.length === 0 ? (
            <EmptySettingsText>
              No store bindings yet. Create a product binding from Products or adopt store products
              from an import preview.
            </EmptySettingsText>
          ) : (
            <div className="overflow-hidden rounded-[12px] border border-[var(--subkit-border)] max-lg:overflow-x-auto">
              <div className="min-w-[920px]">
                <div className="grid grid-cols-[0.7fr_1fr_1fr_1.35fr_1fr_0.85fr] gap-[12px] border-b border-[var(--subkit-border)] bg-[var(--subkit-panel-2)] px-[14px] py-[11px] text-[11px] font-semibold tracking-[0.04em] text-[var(--subkit-faint)] uppercase">
                  <div>Store</div>
                  <div>Product</div>
                  <div>Plan</div>
                ...
                {storeSync.bindings.map((binding) => (
                  <div
                    className="grid grid-cols-[0.7fr_1fr_1fr_1.35fr_1fr_0.85fr] items-center gap-[12px] border-b border-[var(--subkit-border)] px-[14px] py-[12px] last:border-b-0"
                    key={binding.id}
                  >
                    <StoreBadge store={binding.store} />
                    <div className="min-w-0 font-mono text-[12px] text-[var(--subkit-dim)]">
                      {binding.productKey}
                    </div>
                    <div className="min-w-0 font-mono text-[12px] text-[var(--subkit-dim)]">
                      {binding.planKey}
                    ...
                    <div className="min-w-0">
                      <div className="truncate font-mono text-[12px] text-[var(--subkit-text)]">
                        {binding.externalProductId}
                      </div>
                      <div className="truncate font-mono text-[11px] text-[var(--subkit-faint)]">
                        {binding.externalBasePlanId === ''
                          ? binding.environment
                          : `${binding.externalBasePlanId} · ${binding.environment}`}
                      </div>
                    </div>
                    <SoftTag tone="muted">{binding.syncDirection.replaceAll('_', ' ')}</SoftTag>
                    <div className="flex justify-end">
                      <StatusLabel
                        label={binding.bindingStatus.replaceAll('_', ' ')}
                        tone={bindingStatusTone(binding.bindingStatus)}
                      />
                    </div>
                  </div>
                ))}
        ...
        <StorePanel title="Integrations">
          {storeSync.integrations.length === 0 ? (
            <EmptySettingsText>
              No app-scoped store integration records yet. Workspace App Store Connect credentials
              can still import Apple data.
            </EmptySettingsText>
          ) : (
            <div className="flex flex-col gap-[10px]">
              {storeSync.integrations.map((integration) => (
                <div
                  className="rounded-[12px] border border-[var(--subkit-border)] bg-[var(--subkit-panel-2)] px-[12px] py-[11px]"
                  key={integration.id}
                >
                  <div className="flex items-center justify-between gap-[12px]">
                    <div className="min-w-0">
                      <div className="text-[13px] font-semibold text-[var(--subkit-text)]">
                        {integration.displayName}
                      </div>
                      <div className="truncate font-mono text-[11.5px] text-[var(--subkit-faint)]">
                        {integration.externalAppId ?? 'no external app id'}
                      </div>
                    </div>
      ...
      <section className="mt-[16px] grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-[16px] max-xl:grid-cols-1">
        <StorePanel title="Drift">
          {storeSync.driftItems.length === 0 ? (
            <EmptySettingsText>
              No drift items detected. Run an import/compare workflow to populate store current
              state.
            </EmptySettingsText>
          ) : (
            <div className="flex flex-col gap-[10px]">
              {storeSync.driftItems.slice(0, 8).map((item) => (
                <div
                  className="rounded-[12px] border border-[var(--subkit-border)] px-[12px] py-[11px]"
                  key={item.id}
                >
                  <div className="flex items-start justify-between gap-[12px]">
                    <div className="min-w-0">
                      <div className="font-mono text-[12px] text-[var(--subkit-text)]">
                        {item.fieldPath}
                      </div>
                      <div className="mt-[3px] truncate text-[12px] text-[var(--subkit-faint)]">
                        {item.bindingLabel} · {item.driftType}
                      </div>
                    </div>
        ...
        <StorePanel title="Imports and sync runs">
          {storeSync.syncRuns.length === 0 && storeSync.snapshots.length === 0 ? (
            <EmptySettingsText>
              No sync history yet. Use App Settings to fetch Apple catalog data; future store sync
            ...
            <div className="flex flex-col gap-[12px]">
              {storeSync.syncRuns.length > 0 ? (
                <div>
                  <div className="mb-[8px] text-[11px] font-semibold tracking-[0.04em] text-[var(--subkit-faint)] uppercase">
                    Sync runs
                  </div>
                  <div className="flex flex-col gap-[8px]">
                    {storeSync.syncRuns.map((run) => (
                      <div
                        className="rounded-[10px] border border-[var(--subkit-border)] px-[11px] py-[9px]"
                        key={run.id}
                      >
                        <div className="flex items-center justify-between gap-[10px]">
                          <div className="flex items-center gap-[7px]">
                            <StoreBadge store={run.store} />
                            <span className="font-mono text-[12px] text-[var(--subkit-text)]">
                              {run.mode.replaceAll('_', ' ')}
                            </span>
                          </div>
                          <StatusLabel label={run.status} tone={runStatusTone(run.status)} />
                        </div>
                        <div className="mt-[6px] text-[12px] text-[var(--subkit-dim)]">
                          {run.summary}
                        </div>
                        {run.errorDetail != null ? (
                          <div className="mt-[5px] text-[12px] text-[var(--subkit-red)]">
                            {run.errorDetail}
                          </div>
                        ) : null}
                        <div className="mt-[6px] text-[11px] text-[var(--subkit-faint)]">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-[8px]">
                    {storeSync.snapshots.map((snapshot) => (
                      <div
                        className="grid grid-cols-[0.7fr_1fr_1.2fr] items-center gap-[10px] rounded-[10px] border border-[var(--subkit-border)] px-[11px] py-[9px]"
                        key={snapshot.id}
                      >
                        ...
                        <div className="min-w-0 text-right">
                          ...
                          <div className="text-[11px] text-[var(--subkit-faint)]">
                            {snapshot.fetchedAt}
                          </div>
                        </div>
                      </div>
      ...
      <StorePanel title="Mutation plans">
        {storeSync.mutationPlans.length === 0 ? (
          <div className="flex flex-col gap-[12px]">
            <EmptySettingsText>
              No mutation plans yet. This is intentional: external store writes are gated behind
              preview, stale checks, and explicit confirmation.
            </EmptySettingsText>
          ...
          <div className="overflow-hidden rounded-[12px] border border-[var(--subkit-border)] max-lg:overflow-x-auto">
            <div className="min-w-[860px]">
              <div className="grid grid-cols-[0.75fr_1fr_1fr_1.6fr_1fr] gap-[12px] border-b border-[var(--subkit-border)] bg-[var(--subkit-panel-2)] px-[14px] py-[11px] text-[11px] font-semibold tracking-[0.04em] text-[var(--subkit-faint)] uppercase">
                <div>Store</div>
                <div>Status</div>
                <div>Risk</div>
                <div>Summary</div>
              ...
              {storeSync.mutationPlans.map((plan) => (
                <div
                  className="grid grid-cols-[0.75fr_1fr_1fr_1.6fr_1fr] items-center gap-[12px] border-b border-[var(--subkit-border)] px-[14px] py-[12px] last:border-b-0"
                  key={plan.id}
                >
                  ...
                  <div className="truncate text-[12px] text-[var(--subkit-dim)]">
                    {plan.summary}
                  </div>
                  <div className="truncate font-mono text-[11.5px] text-[var(--subkit-faint)]">
                    {plan.previewHash}
...
function StoreMetric({
  ...
  return (
    <div className="rounded-[12px] border border-[var(--subkit-border)] bg-[var(--subkit-panel)] px-[14px] py-[13px]">
      <div className="text-[11px] font-semibold tracking-[0.04em] text-[var(--subkit-faint)] uppercase">
        {label}
      </div>
...
function StoreBadge({ store }: { store: StoreSyncStore }) {
  return (
    <SoftTag tone={store === 'apple' ? 'muted' : 'success'}>
...
function DriftValue({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[9px] border border-[var(--subkit-border)] bg-[var(--subkit-panel-2)] px-[10px] py-[8px]">
      <div className="mb-[4px] text-[10.5px] font-semibold tracking-[0.04em] text-[var(--subkit-faint)] uppercase">
        {label}
      </div>
      <div className="font-mono text-[11.5px] break-words text-[var(--subkit-dim)]">{value}</div>
    </div>
  )
}

function bindingStatusTone(
  ...
  if (
    status === 'planned' ||
    status === 'drifted' ||
    status === 'missing_in_store' ||
    status === 'missing_in_subkit'
  )
    return 'warning'
