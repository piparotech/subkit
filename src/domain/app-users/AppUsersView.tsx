...
export function AppUsersView({
  ...
  return (
    <section className="animate-[subkit-fade-in_200ms_ease] px-[32px] py-[28px] max-md:px-[18px]">
      <ViewTitle
        description="End users of this app, identified by App User ID and resolved through SubKit entitlement grants."
        title="App Users"
      />
      <div className="mt-[20px] overflow-hidden rounded-[14px] border border-[var(--subkit-border)] bg-[var(--subkit-panel)] max-lg:overflow-x-auto">
        <div className="min-w-[980px]">
          <div className="grid grid-cols-[1.5fr_1.2fr_1.25fr_1fr_1fr_0.8fr] gap-[14px] border-b border-[var(--subkit-border)] bg-[var(--subkit-panel-2)] px-[18px] py-[12px] text-[11px] font-semibold tracking-[0.04em] text-[var(--subkit-faint)] uppercase">
            <div>App User ID</div>
            <div>Country</div>
          ...
          {appUsers.map((appUser) => (
            <button
              ...
              type="button"
            >
              <div className="truncate font-mono text-[12.5px] text-[var(--subkit-text)]">
                {appUser.appUserId}
              </div>
              ...
              <div>
                <SoftTag tone={appUser.primaryEntitlement === '—' ? 'muted' : 'success'}>
