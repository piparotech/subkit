...
export function CredentialForm({
  ...
  return (
    <div className="flex flex-col gap-[12px]">
      ...
      <div>
        <label className="mb-[7px] block text-[12.5px] font-semibold text-[var(--subs-text)]" htmlFor="app-store-connect-private-key">
          Private .p8 key file
        </label>
        <div className="rounded-[12px] border border-dashed border-[var(--subs-border-2)] bg-[var(--subs-panel-2)] p-[14px]">
          <input
            accept=".p8"
            className="block w-full cursor-pointer rounded-[9px] border border-[var(--subs-border)] bg-[var(--subs-panel)] px-[10px] py-[8px] text-[12.5px] text-[var(--subs-text)] file:mr-[10px] file:cursor-pointer file:rounded-[7px] file:border-0 file:bg-[var(--subs-text)] file:px-[10px] file:py-[6px] file:text-[12px] file:font-semibold file:text-white"
            id="app-store-connect-private-key"
            onChange={handlePrivateKeyFile}
            type="file"
          />
          <div className="mt-[8px] text-[12px] text-[var(--subs-dim)]">
          ...
          {privateKeyUploadError != null ? <div className="mt-[6px] text-[12px] font-semibold text-[var(--subs-red)]">{privateKeyUploadError}</div> : null}
        </div>
      </div>
    </div>
  )
