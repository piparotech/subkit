...
- **[Expo / React Native SDK](/expo/overview/)** — Public app-bound SDK key. Render
  offerings, run purchases, and read entitlements on-device.
- **[Node.js backend SDK](/node/overview/)** — Server key with scoped capabilities.
  Create customers, contracts, payments, seats, and check entitlements.
- **[Runtime API](/reference/overview/)** — The HTTP surface the mobile SDK calls:
  offerings, customer info, entitlement checks, and purchase reconcile.
- **[Server API](/reference/overview/)** — Tenant/app-scoped, capability-gated HTTP
